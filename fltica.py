...
try:
    ...
    quit("""\033[91mERREUR:\nVeuillez installer ou mettre à jour Pillow.\n\n
            ERROR:\nPlease install or update Pillow.\n\n
            エラー:\nPillowをインストールまたは更新してください。\033[0m""")
...
class Button:
    def __init__(self, posx, posy, text, function, color='black', font="Helvetica", hoverColor='Red', size=24, ancrage='w'):
        """
        ...
        self.lenght = taille_texte(text, font)[0]
        self.pulse = 0
        self.ancrage = ancrage
        self.color = color
        self.pulseBigger = True
        self.bgImg = None
        texte(self.posx, self.posy, self.text, police=self.font, taille=self.size, couleur=self.color, tag='a'+str(id(self)), ancrage=self.ancrage)
        
    def update(self, tev):
        if ((abscisse_souris() < self.posx+self.lenght and abscisse_souris() > self.posx-(self.lenght if self.ancrage == 'center' else 0)) and 
        (ordonnee_souris() < self.posy+30 and ordonnee_souris() > self.posy-30)):
            if tev == 'ClicGauche':
                jouerSon('sounds/click.mp3')
                self.func()
                return ''
            self.isHovered = True
            efface('a'+str(id(self)))
            if self.bgImg:
                efface('bg'+str(id(self)))
                image(self.posx, self.posy, self.bgHover, self.bgWidth, self.bgHeight, tag='bg'+str(id(self)), ancrage=self.ancrage)
            texte(self.posx, self.posy, self.text,self.hoverColor , police=self.font, taille=self.size+self.pulse, tag='a'+str(id(self)), ancrage=self.ancrage) # 'a'+ is requiered for fltk 
            ...
            if self.bgImg:
                efface('bg'+str(id(self)))
                image(self.posx, self.posy, self.bgImg, self.bgWidth, self.bgHeight, tag='bg'+str(id(self)), ancrage=self.ancrage)
            texte(self.posx, self.posy, self.text, couleur=self.color, police=self.font, taille=self.size, tag='a'+str(id(self)), ancrage=self.ancrage)
        return tev
    
    def setBgImg(self, file,fileHover=None, customLenght=None, width=None, height=None):
        self.bgImg = file
        self.bgHover = fileHover
        self.bgWidth = width
        self.bgHeight = height
        if customLenght:
            self.lenght = customLenght
        image(self.posx, self.posy, self.bgImg, self.bgWidth, self.bgHeight, tag='bg'+str(id(self)), ancrage=self.ancrage)
        texte(self.posx, self.posy, self.text, police=self.font, taille=self.size, couleur=self.color, tag='a'+str(id(self)), ancrage=self.ancrage)


class TextBox:
    def __init__(self, x, y, width=400, height=40, anchor = 'nw', tags=''):
        self.e1 = tk.Entry(__canevas.root)
        self.c =  __canevas.canvas.create_window(x,y,width=width, height=height,anchor = anchor, tags=tags, window=self.e1)
...
def image(x, y, fichier, width=None, height=None, ancrage='center', tag=''):
    """
    ...
    img_resized = img.resize((width, height), 0)
...
def animated_image(x, y, width, height, fichier, last_frame=None, ancrage='center'):
    """
    Affiche l'image contenue dans ``fichier`` avec ``(x, y)`` comme centre. Les
    valeurs possibles du point d'ancrage sont ``'center'``, ``'nw'``, etc.
    :param float x: abscisse du point d'ancrage
    ...
    :param ancrage: position du point d'ancrage par rapport à l'image
    :param str tag: étiquette d'objet (défaut : pas d'étiquette)
    :param str last_frame: image à afficher une fois l'animation finie
    :return: identificateur d'objet
    """

    pil_image = Image.open(fichier)

    # Get frame duration, assuming all frame durations are the same
    duration = pil_image.info.get('duration', None)   # None for WEBP
    if duration is None:
        with open(fichier, 'rb') as binfile:
            data = binfile.read()
    ...
    for frame in ImageSequence.Iterator(pil_image):
        cp = frame.copy()
        frame_list.append(cp.convert('RGBA').resize((width, height), 0))
    ...
    """
    Could be used in the futur for infinite animation

    def show_animation():
        global after_id
        if 'delme2' in __img:
            del __img['delme2']
        __canevas.canvas.delete('delme2')
        after_id = __canevas.canvas.after(duration, show_animation)
        img = next(tkframe_sequence)
        __canevas.canvas.create_image(0,0,image=img,tag='delme2')"""
    

    def stop_animation(*event):
        __canevas.canvas.after_cancel(after_id)
        if last_frame:
            return image(x, y, width, height, last_frame, ancrage)

    def run_animation_once():
        global after_id
        if 'delme' in __img:
            del __img['delme']
        __canevas.canvas.delete('delme')
        after_id = __canevas.canvas.after(duration, run_animation_once)
        try:
            img = next(tkframe_iterator)
        except StopIteration:
            stop_animation()
        else:
           __canevas.canvas.create_image(x, y, image=img, anchor=ancrage, tag='delme')
